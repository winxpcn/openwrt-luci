			...
			%>
				<div class="tr cbi-rowstyle-<%=i % 2 + 1%>">
					...
					<div class="td cbi-section-actions">
						<div>
							<input type="button" class="cbi-button cbi-button-reload" onclick="iface_reconnect('<%=net[1]%>')" title="<%:Reconnect this interface%>" value="<%:Restart%>"<%=ifattr(disabled or dynamic, "disabled", "disabled")%> />

							<% if disabled then %>
								<input type="hidden" name="cbid.network.<%=net[1]%>.__disable__" value="1" />
								<input type="submit" name="cbi.apply" class="cbi-button cbi-button-reset" onclick="this.previousElementSibling.value='0'" title="<%:Reconnect this interface%>" value="<%:Connect%>"<%=ifattr(dynamic, "disabled", "disabled")%> />
							<% else %>
