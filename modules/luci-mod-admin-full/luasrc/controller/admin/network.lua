...
function diag_ping(addr, network)
	if network then
		...
		if device then
			diag_command("ping -4 -c 5 -W 1 -I " .. device .. " %s 2>&1", addr)
			return
		end
	end
	diag_command("ping -4 -c 5 -W 1 %s 2>&1", addr)
end
...
function diag_traceroute(addr, network)
	if network then
		local device = get_l3_device_by_network(network)
		if device then
			diag_command("traceroute -4 -q 1 -w 1 -n -i " .. device .. " %s 2>&1", addr)
			return
		end
	end
	diag_command("traceroute -4 -q 1 -w 1 -n %s 2>&1", addr)
end
...
function diag_ping6(addr, network)
	if network then
		local device = get_l3_device_by_network(network)
		if device then
			diag_command("ping6 -c 5 -I " .. device .. " %s 2>&1", addr)
			return
		end
	end
...
function diag_traceroute6(addr, network)
	if network then
		local device = get_l3_device_by_network(network)
		if device then
			diag_command("traceroute6 -q 1 -w 2 -n -i " .. device .. " %s 2>&1", addr)
			return
		end
	end
